...
<div>
	...
	<div class="btm-page-area">
		<div class="pagination">
			<a style="display: <%=pg.getCurPage()==1 ? "none":"block" %>;" href="UsersManagement?p=<%=(pg.getCurPage() - 1)%>">&laquo;</a>
			<%for(int i = 0; i < pg.getTotalPage(); i++){ 
				if(pg.getCurPage() == (i + 1)){
			%>
				<a class="active" href="UsersManagement?p=<%=(i+1) %>"><%=(i+1) %></a>
			<%  } else {%>
				<a href="UsersManagement?p=<%=(i+1) %>"><%=(i+1) %></a>
			<% 
				}
			  }
			%>
			<a style="display: <%=pg.getCurPage()==pg.getTotalPage() ? "none":"block" %>;" href="UsersManagement?p=<%=(pg.getCurPage() + 1)%>">&raquo;</a>
		</div>
	</div>
</div>
</body>
