...
<%
	...
	PageBean<Role> pg = (PageBean<Role>) request.getAttribute("page");
%>
<!DOCTYPE html>
<html>
<head>
...
<div>
	<div class="top-search-area">
		<form action="RolesGet.rl" method="post">
			<span class="sh-txt">角色名:</span> <input name="roleName" class="sh" type="text"/>
			<input class="btn btn-sh" type="submit" value="查询">
		</form>
		<a href="<%=basePath %>/jsp/bRoleAdd.jsp"><button class="btn btn-add">Add</button></a>
	</div>
	<div class="mid-table-area">
		<table class="show-table">
            ...
            <tr>
                ...
                <td><%=pg.getList().get(i).getRemark() %></td>
                <td>
                	<%if(pg.getList().get(i).getIsDelete() == 1){ %>
                		<a onclick="recoverRole(<%=pg.getList().get(i).getId() %>)" class="a-rv">恢复</a>
                	<%} else { %>
                		<a href="RoleUpdateGet.rl?id=<%=pg.getList().get(i).getId() %>" class="a-ud">修改</a> |
                		<a onclick="deleteRole(<%=pg.getList().get(i).getId() %>)" class="a-lk">删除</a>
                	<%}%>
                </td>
	...
	<div class="btm-page-area">
		<div class="pagination">
			<a style="display: <%=pg.getCurPage()==1 ? "none":"block" %>;" href="RolesGet.rl?p=<%=(pg.getCurPage() - 1)%>">&laquo;</a>
			<%for(int i = 0; i < pg.getTotalPage(); i++){ 
				if(pg.getCurPage() == (i + 1)){
			%>
				<a class="active" href="RolesGet.rl?p=<%=(i+1) %>"><%=(i+1) %></a>
			<%  } else {%>
				<a href="RolesGet.rl?p=<%=(i+1) %>"><%=(i+1) %></a>
			<% 
				}
			  }
			%>
			<a style="display: <%=pg.getCurPage()==pg.getTotalPage() ? "none":"block" %>;" href="RolesGet.rl?p=<%=(pg.getCurPage() + 1)%>">&raquo;</a>
		</div>
	</div>
</div>
</body>
